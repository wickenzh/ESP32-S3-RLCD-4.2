...
- `v0.0.61`：优化多条天气预警的显示规则；当接口同时返回多个预警时，按颜色等级选择最高等级的一条显示，优先级为红色、橙色、黄色、蓝色，等级相同时保留接口返回顺序中的第一条，避免顶部预警条轮播或显示过多信息。
- `v0.0.60`：调整顶部天气预警条内部间距，将警告图标向左靠近边框，并扩大文字右侧留白，使图标、文字和边框的间距更均衡；将 RLCD 显示 SPI 写屏速率从 10 MHz 降到 5 MHz，用于压低秒级局部刷新时的瞬时电流峰值。
