...
- `v1.0.13`：新增临时音频诊断固件，在设置页增加 `音频测试 S0`、`S1`、`S2`、`S3` 四个项目，分别只向单个 TDM slot 播放提示音，用于确认单扬声器实际取用的音频槽位；整点报时默认仍使用全槽播放。
- `v1.0.12`：针对单扬声器继续修正整点报时音色差异，将提示音转为 `24kHz / 4-slot TDM / 16-bit PCM`，并把 mono 音频填入全部 4 个 TDM slot，避免硬件实际取用不同 slot 时声音不完整或音色异常。
- `v1.0.11`：整点报时提示音改回厂家音频例程使用的 `24kHz / stereo / 16-bit PCM` 播放链路，并取消播放时强制 384 MCLK，让 ES8311/TDM 走原例程验证过的时钟配置；音量保持上一版水平。
